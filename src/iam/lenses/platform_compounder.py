...
class PlatformCompounderLens(BaseLens):
    ...
    def compute(self, security: Security) -> LensResult:
        ...
        if (
            hasattr(f, "operating_margin_history")
            and f.operating_margin_history
            and len(f.operating_margin_history) >= 2
        ):
        ...
        return LensResult(
            ...
            assumptions={
                "wacc": wacc,
                "g_high": g_high,
                "base_g_terminal": base_g_terminal,
                "g_terminal": g_terminal,
                "margin_expansion_annual": margin_expansion_annual,
                "n": float(n),
            },
        )
