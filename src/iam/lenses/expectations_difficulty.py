...
class ExpectationsDifficultyLens(BaseLens):
    ...
    def compute(self, security: Security) -> LensResult:
        ...
        return LensResult(
            lens_name=self.name,
            fair_value_low=None, fair_value_high=None, implied_move_pct=None,
            confidence=conf, narrative=narrative,
            assumptions={"implied_growth": implied_g if implied_g is not None else 0.0, 
