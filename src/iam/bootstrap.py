...
def check_python_version():
    ...
    if sys.version_info < REQUIRED_PYTHON_VERSION:
        print(f"[!] Institutional Alpha requires Python {REQUIRED_PYTHON_VERSION[0]}.{REQUIRED_PYTHON_VERSION[1]}+")
...
def check_dependencies():
    ...
    try:
        ...
        import pydantic
        import yaml
        print("[âœ“] Core dependencies detected")
        return True
    except ImportError:
...
def init_config():
    ...
    if not user_config.exists() and example_config.exists():
        ...
        print("[âœ“] Created user configuration from template (config.yml)")
