...
from iam.ui.scene import Plane, Marker
...
class DCFValuationSurface(SurfaceModel):
    ...
    def __init__(self, security: Security, base_discount_rate: float=0.09, base_roe: float=0.15, n_years: int=10, terminal_growth: float=0.025):
        ...
        if self.revenue_ttm <= 0:
            ni = f.net_income_ttm if f.net_income_ttm and f.net_income_ttm > 0 else f.fcf_ttm or 0.0
    ...
    def generate_z_grid(self) -> List[List[float]]:
        ...
        self.base_fair_value = _present_value_two_stage(
            base_ni=base_ni_per_share, g_high=self.base_g, n=self.n, 
            g_terminal=self.g_term, r=self.r, roe=self.roe
        )
    ...
    def get_planes(self) -> List[Plane]:
        return [Plane(z=self.market_price, symbol='~', x_range=(self.x_min, self.x_max), y_range=(self.y_min, self.y_max))]

    def get_markers(self) -> List[Marker]:
        return [Marker(x=self.base_g, y=self.base_m, z=self.base_fair_value, symbol='X', label='IAM Base')]
