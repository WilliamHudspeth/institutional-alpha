...
class StooqSource:
    ...
    def get_price_history(self, ticker: str, start: datetime, end: datetime) -> pd.Series:
        ...
        try:
            df = pd.read_csv(
                url, header=None, names=["Date", "Open", "High", "Low", "Close", "Volume"]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prefetch", action="store_true", help="Download and cache all data for backtesting"
    )
    parser.add_argument("--universe", type=str, help="CSV file with ticker column (default: SP500)")
    parser.add_argument(
        "--start_year",
        type=int,
        default=2000,
    ...
    parser.add_argument(
        "--end_year", type=int, default=2026, help="End year for historical data (default: 2026)"
    )
