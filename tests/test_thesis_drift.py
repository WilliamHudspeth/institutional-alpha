...
def test_drift_report_degrade_levels():
    ...
    # Low severity breach
    b1 = ConstraintBreach(
        id="c1", metric="roic_durability", comparator=">=", bound=0.15, actual=0.10,
        severity=1, supports=None, note="test"
    )
    ...
    b2 = ConstraintBreach(
        id="c2", metric="fragility", comparator="<=", bound=0.5, actual=0.6,
        severity=2, supports="Bull", note="test2"
    )
...
def test_drift_detector_evaluate():
    ...
    constraints = [
        ThesisConstraint(id="roic", metric="roic_durability", comparator=">=", bound=0.15, severity=1),
        ThesisConstraint(id="frag", metric="fragility", comparator="<=", bound=0.50, severity=1),
        ThesisConstraint(id="growth", metric="revenue_growth_ttm", comparator=">=", bound=0.05, severity=1),
        ThesisConstraint(id="leverage", metric="debt_to_ebitda", comparator="<=", bound=3.0, severity=2),
    ]
...
def test_drift_detector_skipped():
    ...
    detector = DriftDetector()
    report = detector.evaluate(ticker="AAPL", constraints=constraints, business_reality=None, fundamentals=None)
